...
model {
	...
	target += gamma_lpdf(measureError | 1.0^2/10, 1.0/10); // Gives a mean  of 10 and variance of 1000
	...
	for (i in 1:n_indiv)
	{
		for (j in 2:nbYearsPerIndiv[i]) // Loop for all years but the first (which is the parent of indiv i)
		{
			...
			// if (is_inf(expected_latent_dbh[k]))
			// 	print(k);
		}
	...
	// target += normal_lpdf(latentState[parentsObs_index] | Y_generated_0, processError);
	...
	target += normal_lpdf(Yobs[parents_index] | latentState[parentsObs_index], measureError);

	// Compare true (hidden/latent) children with observed children
	target += normal_lpdf(Yobs[children_index] | latentState[childrenObs_index], measureError);
}
