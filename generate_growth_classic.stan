...
generated quantities {
	...
	
	{
		...
		for (j in 1:n_indiv_new)
			current_dbh[j, 1] = normalised_dbh0[j];
		...
		for (j in 1:n_indiv_new)
		{
			...
			meanlog = growth(normalised_dbh0[j], x_r_avg[j,], averageGrowth, dbh_slope, dbh_slope2,
				pr_slope, pr_slope2 , tas_slope, tas_slope2, ph_slope, ph_slope2, competition_slope);
			simulatedGrowth_avg_clim_avg[j] = n_years*exp(meanlog + sigmaProc^2/2.0);
			final_dbh[j] = normalised_dbh0[j] + simulatedGrowth_avg_clim_avg[j];
		}
	}
}
