...
plotGrowth = function(species, run, variable, ...)
{
	...
	if (any(tas_ind))
	{
		if (!all(ls_names[tas_ind] %in% c("tas_min", "tas_max", "tas_mean")))
	...
	if (any(pr_ind))
	{
		if (!all(ls_names[pr_ind] %in% c("pr_min", "pr_max", "pr_mean")))
	...
	if (any(ph_ind))
	{
		if (!all(ls_names[ph_ind] %in% c("ph_min", "ph_max", "ph_mean")))
