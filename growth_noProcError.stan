...
model {
	...
	for (i in 1:n_indiv) // Loop over all the individuals
	{
		...
		{
			next_dbh = next_dbh + growth(next_dbh, normalised_precip[climate_index[i] + j - 2],
	...
	target += normal_lpdf(normalised_Yobs[children_index] | computed_latent_child, measureError);
}
